    ...
    <div className="grid gap-3 md:gap-6 xl:grid-cols-[0.92fr_1.08fr]">
      <DashboardCard title="Create platform offer" subtitle="Student-first opportunities feed, not ad spam">
        <div className="grid gap-4">
          <select value={form.categoryId} onChange={(event) => setForm((current) => ({ ...current, categoryId: event.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none">
            <option value="">Select category</option>
            {categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}
          </select>
          <input value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="Offer title" />
          <textarea value={form.shortDescription} onChange={(event) => setForm((current) => ({ ...current, shortDescription: event.target.value }))} className="min-h-28 rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="Short description" />
          <div className="grid gap-4 md:grid-cols-2">
            <input value={form.city} onChange={(event) => setForm((current) => ({ ...current, city: event.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="City" />
            <input type="date" value={form.validUntil} onChange={(event) => setForm((current) => ({ ...current, validUntil: event.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" />
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            <select value={form.ctaLabel} onChange={(event) => setForm((current) => ({ ...current, ctaLabel: event.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none">
              <option value="View Details">View Details</option>
              <option value="Contact">Contact</option>
              <option value="Apply">Apply</option>
            </select>
            <input value={form.ctaUrl} onChange={(event) => setForm((current) => ({ ...current, ctaUrl: event.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="https://..." />
          </div>
          <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 text-sm font-semibold">
