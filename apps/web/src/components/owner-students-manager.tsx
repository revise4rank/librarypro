...
export function OwnerStudentsManager() {
  ...
  const [pickerFloorId, setPickerFloorId] = useState("");
  const [availableSeats, setAvailableSeats] = useState<{ id: string; seat_number: string }[]>([]);

  async function loadFloors() {
    try {
      const res = await apiFetch<{ success: boolean; data: { id: string; name: string }[] }>("/owner/floors");
      setFloors(res.data ?? []);
    } catch { setFloors([]); }
  }

  async function loadAvailableSeats(floorId: string) {
    if (!floorId) { setAvailableSeats([]); return; }
    try {
      const res = await apiFetch<{ success: boolean; data: { id: string; seat_number: string; status: string }[] }>(
        `/owner/seats?floorId=${floorId}&status=AVAILABLE`
      );
  ...
  return (
    <div className="grid gap-3 md:gap-6 xl:grid-cols-[0.92fr_1.08fr]">
      <DashboardCard title="Student desk" subtitle="Create or update assignment only when needed">
        <div className="grid gap-4">
          <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
            <div className="rounded-[1rem] border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4">
        ...
        <form className="grid gap-4" onSubmit={onSubmit}>
          <div className="grid gap-4 md:grid-cols-2">
            <input value={form.fullName} onChange={(e) => setForm((c) => ({ ...c, fullName: e.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="Student name" />
            <input value={form.fatherName} onChange={(e) => setForm((c) => ({ ...c, fatherName: e.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="Father name" />
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            <input value={form.phone} onChange={(e) => setForm((c) => ({ ...c, phone: e.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="Phone number" />
            <input value={form.email} onChange={(e) => setForm((c) => ({ ...c, email: e.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="Email" />
          </div>
          <div className="grid gap-4 md:grid-cols-3">
            {/* Two-step seat picker: Floor → Seat */}
            <div className="grid gap-2">
              <select
                value={pickerFloorId}
                onChange={(e) => {
                  setPickerFloorId(e.target.value);
                  setForm((c) => ({ ...c, seatNumber: "" }));
                  void loadAvailableSeats(e.target.value);
                }}
                className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none text-sm"
              >
              ...
              {pickerFloorId ? (
                <select
                  ...
                  onChange={(e) => setForm((c) => ({ ...c, seatNumber: e.target.value }))}
                  className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none text-sm"
                >
                  <option value="">Select seat</option>
                  {availableSeats.map((s) => (
                    <option key={s.id} value={s.seat_number}>Seat {s.seat_number}</option>
                  ))}
                  {availableSeats.length === 0 ? <option disabled>No available seats</option> : null}
                </select>
              ) : (
                <input
                  value={form.seatNumber}
                  onChange={(e) => setForm((c) => ({ ...c, seatNumber: e.target.value }))}
                  className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none text-sm"
                  placeholder="Or type seat number"
                />
              )}
            </div>
            <input value={form.planName} onChange={(e) => setForm((c) => ({ ...c, planName: e.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="Plan name" />
            <input value={form.planPrice} onChange={(e) => setForm((c) => ({ ...c, planPrice: e.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="Plan price" />
          </div>
          <div className="grid gap-4 md:grid-cols-4">
            <input type="date" value={form.startsAt} onChange={(e) => handleStartDateChange(e.target.value)} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" />
            <input value={form.durationMonths} onChange={(e) => handleDurationChange(e.target.value)} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="Months" type="number" min="1" />
            <input type="date" value={form.endsAt} onChange={(e) => setForm((c) => ({ ...c, endsAt: e.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" />
            <select value={form.paymentStatus} onChange={(e) => setForm((c) => ({ ...c, paymentStatus: e.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none">
              <option value="PENDING">Pending</option>
              <option value="PAID">Paid</option>
              <option value="DUE">Due</option>
            </select>
          </div>
          <div className="rounded-[1.25rem] border border-slate-200 bg-slate-50 px-3 py-2.5 md:px-4 md:py-4 text-sm text-slate-700">
          ...
          <textarea value={form.notes} onChange={(e) => setForm((c) => ({ ...c, notes: e.target.value }))} className="min-h-28 rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="Notes" />
      ...
      <div className="grid gap-3 md:gap-6">
        {selectedStudent ? (
          <DashboardCard title="Selected student" subtitle="Use this compact card before editing or coaching">
            <div className="grid gap-4">
              <div className="flex flex-wrap items-start justify-between gap-3 rounded-[1rem] border border-slate-200 bg-slate-50 px-3 py-2.5 md:px-4 md:py-4">
                <div>
              ...
              <>
                ...
                {!loading ? (
                  <div className="overflow-hidden rounded-[1.5rem] border border-slate-200">
                    ...
                    {rows.map((student) => {
                      const isSelected = selectedAssignmentId === student.assignment_id;
                      return (
                        <div key={student.assignment_id} className="border-t border-slate-200 first:border-t-0">
                          {/* Mobile: compact accordion row */}
                          <button
                            type="button"
                            onClick={() => setSelectedAssignmentId(isSelected ? null : student.assignment_id)}
                            className={`flex w-full items-center gap-3 px-3 py-3 text-left md:hidden ${isSelected ? "bg-slate-950" : "bg-white"}`}
                          >
                            <span className={`flex-shrink-0 rounded-lg px-2 py-1 text-xs font-black ${isSelected ? "bg-white/10 text-white" : "bg-slate-100 text-slate-700"}`}>
                              {student.seat_number ?? "—"}
                            </span>
                            <span className={`min-w-0 flex-1 truncate text-sm font-bold ${isSelected ? "text-white" : "text-slate-950"}`}>{student.student_name}</span>
                            <span className={`flex-shrink-0 rounded-full px-2 py-1 text-[10px] font-black ${
                              isSelected ? "bg-white/10 text-white" : student.payment_status === "PAID" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"
                            }`}>{student.payment_status}</span>
                            <span className={`flex-shrink-0 text-xs font-bold ${isSelected ? "text-slate-300" : "text-slate-600"}`}>₹{student.due_amount}</span>
                            <span className={`flex-shrink-0 text-xs ${isSelected ? "text-slate-300" : "text-slate-400"}`}>{isSelected ? "▲" : "▼"}</span>
                          </button>
                          {/* Mobile: expanded section */}
                          {isSelected ? (
                            <div className="border-t border-white/10 bg-slate-900 px-3 py-3 md:hidden">
                              <p className="text-xs text-slate-300">{student.student_phone ?? student.student_email ?? "-"} · {student.plan_name} · Valid {student.ends_at}</p>
                              <div className="mt-2.5 flex flex-wrap gap-2">
                                <button type="button" onClick={() => loadIntoForm(student)} className="rounded-full bg-white/10 px-3 py-1 text-xs font-bold text-white">Edit</button>
                                <button type="button" onClick={() => void loadProductivity(student.student_user_id, student.student_name)} className="rounded-full bg-cyan-500/20 px-3 py-1 text-xs font-bold text-cyan-100">Focus</button>
                                <Link href={`/owner/students/${student.student_user_id}?name=${encodeURIComponent(student.student_name)}`} className="rounded-full bg-indigo-500/20 px-3 py-1 text-xs font-bold text-indigo-100">Open</Link>
                                <button type="button" onClick={() => void onDelete(student.assignment_id)} className="rounded-full bg-rose-500/20 px-3 py-1 text-xs font-bold text-rose-100">Delete</button>
                              </div>
                            </div>
                          ) : null}
                          {/* Desktop: original full grid row */}
                          <button
                            type="button"
                            onClick={() => setSelectedAssignmentId(isSelected ? null : student.assignment_id)}
                            className={`hidden w-full border-t-0 px-4 py-3 text-left text-sm md:block ${isSelected ? "bg-slate-950 text-white" : "bg-white"}`}
                          >
                            <div className="grid gap-3 md:grid-cols-[1.3fr_0.7fr_1fr_0.85fr_0.75fr_0.8fr]">
                              <div className="min-w-0">
                                <p className={`font-bold ${isSelected ? "text-white" : "text-slate-950"}`}>{student.student_name}</p>
                                <p className={isSelected ? "text-slate-300" : "text-slate-500"}>{student.student_phone ?? student.student_email ?? "-"}</p>
                                <p className={`text-xs ${isSelected ? "text-cyan-200" : "text-[var(--lp-primary)]"}`}>Login ID: {student.student_code ?? student.student_phone ?? "-"}</p>
                                <div className="mt-2 flex gap-2">
                                  <button type="button" onClick={(e) => { e.stopPropagation(); loadIntoForm(student); }} className={`rounded-full px-2 py-1 text-xs font-bold ${isSelected ? "bg-white/10 text-white" : "bg-slate-100 text-slate-700"}`}>Edit</button>
                                  <button type="button" onClick={(e) => { e.stopPropagation(); void loadProductivity(student.student_user_id, student.student_name); }} className={`rounded-full px-2 py-1 text-xs font-bold ${isSelected ? "bg-cyan-500/20 text-cyan-100" : "bg-cyan-100 text-cyan-700"}`}>Focus</button>
                                  <Link href={`/owner/students/${student.student_user_id}?name=${encodeURIComponent(student.student_name)}`} onClick={(e) => e.stopPropagation()} className={`rounded-full px-2 py-1 text-xs font-bold ${isSelected ? "bg-indigo-500/20 text-indigo-100" : "bg-indigo-100 text-indigo-700"}`}>Open</Link>
                                  <button type="button" onClick={(e) => { e.stopPropagation(); void onDelete(student.assignment_id); }} className={`rounded-full px-2 py-1 text-xs font-bold ${isSelected ? "bg-rose-500/20 text-rose-100" : "bg-rose-100 text-rose-700"}`}>Delete</button>
                                </div>
                              </div>
                              <span className={`font-semibold ${isSelected ? "text-white" : "text-slate-700"}`}>{student.seat_number ?? "-"}</span>
                              <span className={`font-semibold ${isSelected ? "text-white" : "text-slate-700"}`}>{student.plan_name}</span>
                              <span className={`font-semibold ${isSelected ? "text-white" : "text-slate-700"}`}>{student.ends_at}</span>
                              <span className={`w-fit rounded-full px-2 py-1 text-[10px] font-black ${isSelected ? "bg-white/10 text-white" : student.payment_status === "PAID" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>{student.payment_status}</span>
                              <span className={`font-bold ${isSelected ? "text-white" : "text-slate-950"}`}>₹{student.due_amount}</span>
                            </div>
                          </button>
                        </div>
                      );
