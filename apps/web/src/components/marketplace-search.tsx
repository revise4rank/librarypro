...
export function MarketplaceSearch() {
  ...
  useEffect(() => {
    ...
    if (compareLibraryIds.length > 0) nextParams.set("compare", compareLibraryIds.join(","));
    if (sortBy !== "relevance") nextParams.set("sort", sortBy);
    const nextQuery = nextParams.toString();
    router.replace(nextQuery ? `${pathname}?${nextQuery}` : pathname, { scroll: false });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [compareLibraryIds, sortBy]);
                  ...
                  <div className="p-5 md:p-6">
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-2 md:grid-cols-3">
                      <div className="rounded-[1.2rem] bg-[#f4faf5] p-4">
                        ...
                        <p className="mt-2 text-lg font-extrabold text-[var(--lp-text)]">
                          {(library.reviews ?? 0) === 0 ? "No reviews yet" : `${Number(library.rating ?? 0).toFixed(1)}/5`}
                        </p>
                      </div>
                      <div className="rounded-[1.2rem] bg-[#f4faf5] p-4">
                        <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-[var(--lp-accent)]">Reviews</p>
                        <p className="mt-2 text-lg font-extrabold text-[var(--lp-text)]">
                          {(library.reviews ?? 0) === 0 ? "Be the first" : library.reviews}
                        </p>
                      </div>
                      <div className="rounded-[1.2rem] bg-[#f4faf5] p-4">
                        <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-[var(--lp-accent)]">Quietness</p>
                        <p className="mt-2 text-lg font-extrabold text-[var(--lp-text)]">{library.quietness ?? "N/A"}</p>
                      </div>
                    </div>
                    ...
                    <div className="mt-6 grid gap-3 sm:grid-cols-2">
                      ...
                      <a href={`https://${library.subdomain}.nextlib.in`} target="_blank" rel="noopener noreferrer" className="rounded-[1.25rem] border border-[var(--lp-border)] bg-[var(--lp-surface)] px-5 py-3.5 text-center text-sm font-semibold text-[var(--lp-primary)]">
                        Go To Library Site ↗
                      </a>
                    </div>
