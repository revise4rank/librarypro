...
export function StudentSeatManager() {
  ...
  return (
    <div className="grid gap-3 md:gap-6 xl:grid-cols-[1fr_1fr]">
      <DashboardCard title="Assignment summary" subtitle="Current active subscription and seat">
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-[1.5rem] bg-slate-50 p-5">
            ...
            <p className="mt-3 text-2xl md:text-3xl font-black text-slate-950">{data.assignment?.seat_number ?? "-"}</p>
          </div>
          ...
          <div className="rounded-[1.5rem] bg-amber-50 p-5">
            ...
            <p className="mt-3 text-xl font-black text-slate-950">{formatDate(data.assignment?.ends_at)}</p>
          </div>
          <div className="rounded-[1.5rem] bg-cyan-50 p-5">
            ...
            <p className="mt-3 text-xl font-black text-slate-950">{formatDate(data.upcomingDueDate)}</p>
          </div>
        </div>
      </DashboardCard>
          ...
          {showLibraryInfo ? (
            <div className="space-y-4 text-sm leading-7 text-slate-700">
              ...
              <div className="flex items-center gap-2">
                <span className="font-black text-slate-950">Password:</span>
                <span>{showWifiPassword ? (data.library?.wifi_password ?? "-") : "••••••••"}</span>
                <button
                  type="button"
