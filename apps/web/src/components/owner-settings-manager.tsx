...
export function OwnerSettingsManager() {
  ...
  return (
    <div className="grid gap-3 md:gap-6 xl:grid-cols-2">
      ...
      <Surface title="Library profile" subtitle="Basic customer-facing information">
        <form className="grid gap-4" onSubmit={saveSettings}>
          <input value={form.libraryName} onChange={(event) => setForm((current) => ({ ...current, libraryName: event.target.value }))} className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="Library name" />
          <input value={form.address} onChange={(event) => setForm((current) => ({ ...current, address: event.target.value }))} className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="Address" />
          <div className="grid gap-4 md:grid-cols-2">
            <input value={form.city} onChange={(event) => setForm((current) => ({ ...current, city: event.target.value }))} className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="City" />
            <input value={form.area} onChange={(event) => setForm((current) => ({ ...current, area: event.target.value }))} className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="Area / Locality" />
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            <input value={form.wifiName} onChange={(event) => setForm((current) => ({ ...current, wifiName: event.target.value }))} className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="WiFi name" />
            <input value={form.wifiPassword} onChange={(event) => setForm((current) => ({ ...current, wifiPassword: event.target.value }))} className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="WiFi password" />
          </div>
          <textarea value={form.noticeMessage} onChange={(event) => setForm((current) => ({ ...current, noticeMessage: event.target.value }))} className="min-h-36 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="Notice message" />
          <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 text-sm font-semibold text-slate-700">
            <input
              type="checkbox"
