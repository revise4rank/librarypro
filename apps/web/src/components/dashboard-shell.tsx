...
export function DashboardShell({
  ...
  const groupedNavItems = navGroups
    ...
    : null;
  const activeGroupItems = groupedNavItems?.find((g) => g.id === activeGroupId)?.items ?? [];
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(236,173,132,0.18),transparent_24%),radial-gradient(circle_at_top_right,rgba(153,214,209,0.18),transparent_22%),linear-gradient(180deg,#fbf6ee_0%,#fffaf3_52%,#f6efe4_100%)] text-[var(--lp-text)]">
      <div className={`grid min-h-screen ${desktopCollapsed ? "lg:grid-cols-[92px_minmax(0,1fr)]" : "lg:grid-cols-[280px_minmax(0,1fr)]"}`}>
        <aside className={`hidden border-r border-[var(--lp-border)] bg-[rgba(255,249,240,0.92)] py-6 text-[var(--lp-text)] transition-[width,padding] duration-200 lg:sticky lg:top-0 lg:flex lg:h-screen lg:flex-col ${desktopCollapsed ? "lg:px-3" : "lg:px-5"}`}>
            ...
            {nav.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`flex items-center ${desktopCollapsed ? "justify-center" : "justify-between"} rounded-xl px-4 py-3 text-sm font-semibold transition ${
                    active
                      ? "bg-[var(--lp-primary)] text-white shadow-md"
          ...
          {!desktopCollapsed ? (
            <div className="mt-8 rounded-xl border border-[var(--lp-border)] bg-[rgba(255,250,244,0.82)] p-3">
        ...
        <div className="min-w-0">
          ...
          {matchedGuide ? (
            <div className="px-4 pb-24 md:px-6 lg:px-8 lg:pb-8">
              <section className="rounded-xl border border-[var(--lp-border)] bg-[rgba(255,250,244,0.72)] px-4 py-3 shadow-sm">
                ...
                {guideOpen ? (
                  <ol className="mt-3 grid gap-2 text-sm text-[var(--lp-text)] lg:grid-cols-3">
                    {matchedGuide.steps.map((step, index) => (
                      <li key={`${matchedGuide.title}-${index}`} className="rounded-lg border border-[var(--lp-border)] bg-white px-3 py-3 leading-6 shadow-sm">
          ...
          <section className="px-3 py-3 pb-24 md:px-6 lg:px-8 lg:py-8 lg:pb-8">{children}</section>
        </div>
      </div>
      ...
      <div className="lg:hidden">
        ...
        <div
          className={`fixed bottom-24 z-40 w-[min(22rem,calc(100vw-1.5rem))] rounded-2xl border border-[var(--lp-border)] bg-[rgba(255,249,241,0.98)] p-4 shadow-lg backdrop-blur transition ${
        ...
        <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-[var(--lp-border)] bg-[rgba(255,249,241,0.98)] px-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] pt-3 backdrop-blur">
          {groupedNavItems ? (
            /* Grouped nav */
            <>
              {(activeGroupId && activeGroupItems.length > 0) ? (
                <div className="mb-2 flex flex-wrap gap-2 rounded-xl border border-[var(--lp-border)] bg-white p-2">
                  {activeGroupItems.map((item) => {
                    ...
                    return (
                      <Link
                        key={item.href}
                        href={item.href}
                        className={`rounded-lg px-3 py-1.5 text-xs font-bold ${active ? "bg-[var(--lp-primary)] text-white" : "bg-slate-100 text-slate-700"}`}
                      >
                        {item.label}
                      </Link>
                    );
              ...
              <div className={`grid gap-2`} style={{ gridTemplateColumns: `repeat(${groupedNavItems.length}, minmax(0, 1fr))` }}>
                {groupedNavItems.map((group) => {
                  const isActive = activeGroupId === group.id || group.items.some((item) => pathname === item.href);
                    ...
                    return (
                      <Link
                        ...
                        className={`flex flex-col items-center justify-center rounded-xl px-2 py-2 text-center ${
                          pathname === group.items[0].href ? "bg-[var(--lp-primary)] text-white" : "bg-white text-[var(--lp-text)]"
                        }`}
                      >
                        <span className="text-base leading-none">{group.icon}</span>
                        <span className="mt-1 text-[10px] font-bold">{group.label}</span>
                      </Link>
                    );
                  }
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center justify-center rounded-xl px-2 py-2 text-center ${
                        isActive ? "bg-[var(--lp-primary)] text-white" : "bg-white text-[var(--lp-text)]"
                      }`}
                    >
                      <span className="text-base leading-none">{group.icon}</span>
                      <span className="mt-1 text-[10px] font-bold">{group.label}</span>
              ...
              {primaryMobileNav.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`flex flex-col items-center justify-center rounded-xl px-2 py-2 text-center text-[11px] font-bold ${
...
export function DashboardCard({
  ...
  children: ReactNode;
  tone?: string;
  size?: "compact" | "default" | "flush";
}) {
  const padding = size === "compact" ? "p-3" : size === "flush" ? "p-0" : "p-3 md:p-5";
  const titleSize = size === "compact" ? "text-base" : "text-lg";
  const bodyMargin = size === "compact" ? "mt-3" : size === "flush" ? "mt-0" : "mt-3 md:mt-5";
  return (
    <section className={`rounded-xl border border-[var(--lp-border)] shadow-sm ${padding} ${tone}`}>
      {size !== "flush" ? (
        <div className="flex flex-col gap-1">
          <h3 className={`font-extrabold text-[var(--lp-text)] ${titleSize}`}>{title}</h3>
          {subtitle && size !== "compact" ? <p className="text-sm text-[var(--lp-muted)]">{subtitle}</p> : null}
        </div>
      ) : null}
      <div className={bodyMargin}>{children}</div>
    </section>
  );
}
