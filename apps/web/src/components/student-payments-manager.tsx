...
export function StudentPaymentsManager() {
  ...
  return (
    <div className="grid gap-3 md:gap-6 xl:grid-cols-[0.85fr_1.15fr]">
      <DashboardCard title="Current billing status" subtitle={`Real student payment summary | Socket ${liveStatus}`}>
        <div className="grid gap-4">
          ...
          <div className="rounded-[1.5rem] bg-amber-50 p-5">
            ...
            <p className="mt-3 text-2xl md:text-3xl font-black text-slate-950">{summary.paymentStatus ?? "UNKNOWN"}</p>
          </div>
        ...
        <div className="grid gap-4">
          ...
          {showHistory ? (
            <div className="space-y-3">
              {rows.map((payment) => (
                <div key={payment.id} className="rounded-[1.25rem] border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4">
