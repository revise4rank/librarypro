...
export function OwnerNotificationsManager() {
  ...
  return (
    <div className="grid gap-3 md:gap-6 xl:grid-cols-[0.84fr_1.16fr]">
      {toast ? (
      ...
      <DashboardCard title="Compose broadcast" subtitle={`Owner to student communication workflow | Socket ${liveStatus}`}>
        <form className="grid gap-4" onSubmit={onSubmit}>
          ...
          <input value={form.title} onChange={(e) => setForm((c) => ({ ...c, title: e.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="Notification title" />
          <div className="grid gap-4 md:grid-cols-2">
            <select value={form.type} onChange={(e) => setForm((c) => ({ ...c, type: e.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none">
              <option value="PAYMENT_REMINDER">Payment reminder</option>
              <option value="EXPIRY_ALERT">Expiry alert</option>
              <option value="GENERAL">General message</option>
            </select>
            <select value={form.audience} onChange={(e) => setForm((c) => ({ ...c, audience: e.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none">
              <option value="ALL_STUDENTS">All students</option>
              <option value="DUE_STUDENTS">Only unpaid students</option>
              <option value="EXPIRING_STUDENTS">Expiring in 3 days</option>
            </select>
          </div>
          <textarea value={form.message} onChange={(e) => setForm((c) => ({ ...c, message: e.target.value }))} className="min-h-36 rounded-2xl border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4 outline-none" placeholder="Write message" />
