...
export function SuperadminDashboardManager() {
  ...
  return (
    <div className="grid gap-3 md:gap-6">
      ...
      <section className="grid gap-3 md:gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <DashboardCard title="Tenant watchlist" subtitle="Who needs admin attention right now">
          <div className="space-y-3">
            {data.watchlist.map((library) => (
              <div key={library.id} className="rounded-[1.25rem] border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4">
        ...
        <DashboardCard title="Recent platform charges" subtitle="Subscription billing stream">
          <div className="space-y-3">
            {data.recentPayments.map((payment) => (
              <div key={payment.id} className="flex items-center justify-between rounded-[1.25rem] border border-slate-200 bg-white px-3 py-2.5 md:px-4 md:py-4">
                <div>
