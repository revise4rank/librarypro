...
export function OwnerAdmissionsManager() {
  ...
  async function load() {
    try {
      const [joinResponse, seatResponse, floorResponse, planResponse] = await Promise.all([
        ...
        apiFetch<{ success: boolean; data: PlanTemplate[] }>("/owner/plan-templates").catch(() => ({ success: true, data: [] as PlanTemplate[] })),
      ]);
      setRequests(joinResponse.data);
      setSeats(seatResponse.data);
      setFloors(floorResponse.data);
      setPlanTemplates(planResponse.data);
    } catch (error) {
                ...
                <div className="mt-4 grid gap-3">
                  ...
                  <div className="grid gap-3 sm:grid-cols-[180px_1fr_140px_auto_auto]">
                    ...
                    {planTemplates.length > 0 ? (
                      <select
                        value={planPrices[request.id] ?? ""}
                        onChange={(event) => {
                          const selected = planTemplates.find((p) => p.id === event.target.value);
                          if (selected) {
                            setPlanPrices((current) => ({ ...current, [request.id]: selected.price }));
                          }
                        }}
                        className="rounded-2xl border border-[var(--lp-border)] bg-white px-4 py-3"
                      >
