    ...
    <div className="grid gap-4 xl:grid-cols-[0.72fr_1.28fr]">
      <DashboardCard title="Admissions flow" subtitle="Onboard students from one place, then allot seats later from the student roster.">
        <div className="grid gap-4">
          <div className="rounded-[1rem] bg-[linear-gradient(135deg,#17314f_0%,#285d6d_100%)] p-4 text-white">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <button
              ...
              className={`rounded-[0.75rem] border px-4 py-3 text-left text-sm font-semibold ${mode === "desk" ? "border-[var(--lp-accent)] bg-[var(--lp-accent-soft)] text-[var(--lp-accent)]" : "border-[var(--lp-border)] bg-white text-[var(--lp-text)]"}`}
            >
              Desk Admission
            </button>
            <button
              ...
              className={`rounded-[0.75rem] border px-4 py-3 text-left text-sm font-semibold ${mode === "requests" ? "border-[var(--lp-accent)] bg-[var(--lp-accent-soft)] text-[var(--lp-accent)]" : "border-[var(--lp-border)] bg-white text-[var(--lp-text)]"}`}
            >
              Join Requests
            </button>
          </div>

          <div className="grid gap-3 rounded-[0.75rem] border border-[var(--lp-border)] bg-white p-4">
          ...
          {mode === "requests" ? (
            <div className="grid gap-3">
              {requests.map((request) => (
                <div key={request.id} className={`rounded-[0.75rem] border p-4 ${selectedRequestId === request.id ? "border-[var(--lp-accent)] bg-[var(--lp-accent-soft)]/40" : "border-[var(--lp-border)] bg-white"}`}>
            ...
            <div className="grid gap-3 rounded-[0.75rem] border border-[var(--lp-border)] bg-white p-4 text-sm text-[var(--lp-text-soft)]">
      ...
      <DashboardCard title={mode === "desk" ? "Desk admission" : "Approve request into admission"} subtitle="The same form powers manual onboarding and request approvals.">
        <form className="grid gap-4" onSubmit={submitAdmission}>
          ...
          <div className="grid gap-3 rounded-[0.75rem] border border-[var(--lp-border)] bg-[var(--lp-surface)] p-4 md:grid-cols-2">
            <select value={form.studentPlanId} onChange={(event) => setForm((current) => ({ ...current, studentPlanId: event.target.value }))} className="rounded-[0.5rem] border border-[var(--lp-border)] bg-white px-4 py-2 outline-none">
              <option value="">Select plan</option>
              {plans.map((plan) => (
                <option key={plan.id} value={plan.id}>{plan.name} • Rs. {Number(plan.base_amount).toLocaleString("en-IN")}</option>
              ))}
            </select>
            <input value={form.couponCode} onChange={(event) => setForm((current) => ({ ...current, couponCode: event.target.value.toUpperCase() }))} className="rounded-[0.5rem] border border-[var(--lp-border)] bg-white px-4 py-2 outline-none" placeholder="Coupon code (optional)" />
            <input type="number" min="0" value={form.planAmountOverride} onChange={(event) => setForm((current) => ({ ...current, planAmountOverride: event.target.value }))} className="rounded-[0.5rem] border border-[var(--lp-border)] bg-white px-4 py-2 outline-none" placeholder="Manual fee override" />
            <input type="number" min="1" value={form.durationMonthsOverride} onChange={(event) => setForm((current) => ({ ...current, durationMonthsOverride: event.target.value }))} className="rounded-[0.5rem] border border-[var(--lp-border)] bg-white px-4 py-2 outline-none" placeholder="Duration override (months)" />
          </div>

          <div className="grid gap-3 md:grid-cols-[1fr_auto]">
            <select value={form.paymentStatus} onChange={(event) => setForm((current) => ({ ...current, paymentStatus: event.target.value as AdmissionFormState["paymentStatus"] }))} className="rounded-[0.5rem] border border-[var(--lp-border)] bg-white px-4 py-2 outline-none">
              <option value="UNPAID">Unpaid</option>
              <option value="DUE">Due</option>
              <option value="PAID">Paid</option>
            </select>
            <div className="rounded-[0.5rem] border border-[var(--lp-border)] bg-white px-4 py-2 text-sm font-semibold text-[var(--lp-text)]">
          ...
          <textarea value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} className="min-h-24 rounded-[0.5rem] border border-[var(--lp-border)] bg-white px-4 py-2 outline-none" placeholder="Operator notes" />

          {selectedPlan ? (
            <div className="rounded-[0.75rem] border border-[var(--lp-border)] bg-[#f8fbff] p-4 text-sm text-[var(--lp-text-soft)]">
