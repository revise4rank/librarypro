    ...
    <div className="grid gap-3 md:gap-6">
      ...
      <section className="grid gap-4 md:grid-cols-2 2xl:grid-cols-4">
        <div className="rounded-[1.75rem] border border-slate-200 bg-white p-5">
          ...
          <p className="mt-3 text-2xl md:text-3xl font-black text-slate-950">{data.badges.length}</p>
        </div>
        <div className="rounded-[1.75rem] border border-slate-200 bg-white p-5">
          <p className="text-xs font-black uppercase tracking-[0.24em] text-slate-400">Longest streak</p>
          <p className="mt-3 text-2xl md:text-3xl font-black text-slate-950">{data.longestStreak} days</p>
        </div>
        <div className="rounded-[1.75rem] border border-slate-200 bg-white p-5">
          <p className="text-xs font-black uppercase tracking-[0.24em] text-slate-400">Total study</p>
          <p className="mt-3 text-2xl md:text-3xl font-black text-slate-950">{data.totalStudyHours} hrs</p>
        </div>
        <div className="rounded-[1.75rem] border border-slate-200 bg-white p-5">
          <p className="text-xs font-black uppercase tracking-[0.24em] text-slate-400">Attendance days</p>
          <p className="mt-3 text-2xl md:text-3xl font-black text-slate-950">{data.attendanceDays}</p>
        </div>
      </section>
