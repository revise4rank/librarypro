...
SET listing_published = TRUE
WHERE is_published = TRUE
  AND show_in_marketplace = TRUE;

DROP MATERIALIZED VIEW IF EXISTS marketplace_search_index;

CREATE MATERIALIZED VIEW marketplace_search_index AS
SELECT
  l.id AS library_id,
...
CREATE INDEX IF NOT EXISTS idx_marketplace_search_index_publish
  ON marketplace_search_index(listing_published, show_in_marketplace, library_status, city);
