...
const envSchema = z.object({
  ...
  jwtAccessTtl: z.string().min(1).default("10d"),
  ...
  apiRateLimitMax: z.coerce.number().int().positive().default(5_000),
  publicReadRateLimitMax: z.coerce.number().int().positive().default(20_000),
  authRateLimitMax: z.coerce.number().int().positive().default(20),
  ...
  razorpayKeySecret: z.string().default(""),
...
export const env = envSchema.parse({
  ...
  jwtAccessTtl: process.env.JWT_ACCESS_TTL ?? "10d",
  ...
  apiRateLimitMax: process.env.API_RATE_LIMIT_MAX ?? 5_000,
  publicReadRateLimitMax: process.env.PUBLIC_READ_RATE_LIMIT_MAX ?? 20_000,
  authRateLimitMax: process.env.AUTH_RATE_LIMIT_MAX ?? 20,
