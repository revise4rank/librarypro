...
export const env = {
  ...
  baseDomain: process.env.BASE_DOMAIN ?? "nextlib.in",
  ...
  otelServiceName: process.env.OTEL_SERVICE_NAME ?? "nextlib-api",
  otelExporterOtlpEndpoint: process.env.OTEL_EXPORTER_OTLP_ENDPOINT ?? "",
  uploadsDir: process.env.UPLOADS_DIR ?? "uploads",
  uploadsProvider: process.env.UPLOADS_PROVIDER ?? "local",
  supabaseUrl: process.env.SUPABASE_URL ?? "",
  supabaseServiceRoleKey: process.env.SUPABASE_SERVICE_ROLE_KEY ?? "",
  supabaseBucket: process.env.SUPABASE_BUCKET ?? "nextlib-assets",
};
