...
export async function listStudentBooks(studentUserId: string) {
  return new ProductivityRepository(requireDb()).listStudentBooks(studentUserId);
}

export async function createStudentBook(input: {
  studentUserId: string;
  libraryId: string | null;
  title: string;
  author?: string;
  ...
  return new ProductivityRepository(requireDb()).createStudentBook({
    ...input,
    status: input.status ?? "READING",
  });
}

export async function updateStudentBook(input: {
  id: string;
  studentUserId: string;
  status?: string;
  currentPage?: number;
  ...
  const row = await new ProductivityRepository(requireDb()).updateStudentBook(input);
  if (!row) {
...
export async function deleteStudentBook(id: string, studentUserId: string) {
  return new ProductivityRepository(requireDb()).deleteStudentBook(id, studentUserId);
}

// ── Study Planner ──────────────────────────────────────────────────────────

export async function listPlannerWeek(studentUserId: string, weekStart: string) {
  return new ProductivityRepository(requireDb()).listPlannerWeek(studentUserId, weekStart);
}

export async function listPlannerMonth(studentUserId: string, month: string) {
  return new ProductivityRepository(requireDb()).listPlannerMonth(studentUserId, month);
}
...
export async function createPlannerEntry(input: {
  ...
  return new ProductivityRepository(requireDb()).createPlannerEntry(input);
}

export async function updatePlannerEntry(input: {
  id: string;
  studentUserId: string;
  ...
  const row = await new ProductivityRepository(requireDb()).updatePlannerEntry(input);
  if (!row) throw new AppError(404, "Planner entry not found", "ENTRY_NOT_FOUND");
  return row;
}

export async function deletePlannerEntry(id: string, studentUserId: string) {
  return new ProductivityRepository(requireDb()).deletePlannerEntry(id, studentUserId);
}

// ── Referral System ────────────────────────────────────────────────────────

function generateReferralCode(userId: string): string {
  // 8-char alphanumeric code derived from userId prefix + random
  const prefix = userId.replace(/-/g, "").slice(0, 4).toUpperCase();
  const rand = Math.random().toString(36).slice(2, 6).toUpperCase();
  return `${prefix}${rand}`;
}

export async function getOrCreateStudentReferralCode(studentUserId: string, libraryId: string | null) {
  const pool = requireDb();
  // Try to find existing code
  const existing = await pool.query<{ id: string; code: string; uses_count: number }>(
    `SELECT id, code, uses_count FROM referral_codes WHERE user_id = $1 AND role = 'student' LIMIT 1`,
    [studentUserId],
  );
  if (existing.rows[0]) {
    const uses = await pool.query<{ referred_user_id: string; referred_at: string }>(
      `SELECT referred_user_id::text, referred_at::text FROM referral_uses WHERE referral_code_id = $1 ORDER BY referred_at DESC`,
      [existing.rows[0].id],
    );
    return { ...existing.rows[0], uses: uses.rows };
  }

  // Create new code
  let code = generateReferralCode(studentUserId);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    code = generateReferralCode(studentUserId + i);
  }

  const created = await pool.query<{ id: string; code: string }>(
    `INSERT INTO referral_codes (user_id, code, role, library_id) VALUES ($1, $2, 'student', $3) RETURNING id, code`,
    [studentUserId, code, libraryId ?? null],
  );
  return { ...created.rows[0], uses_count: 0, uses: [] };
}

export async function toggleFeedLike(postId: string, studentUserId: string) {
  return new ProductivityRepository(requireDb()).toggleFeedLike(postId, studentUserId);
}

export async function getOwnerReferralStats(libraryId: string) {
  const pool = requireDb();
  const result = await pool.query<{
    ...
    `
    SELECT rc.code, u.full_name AS user_name, rc.uses_count, rc.created_at::text
    FROM referral_codes rc
    INNER JOIN users u ON u.id = rc.user_id
    WHERE rc.library_id = $1 AND rc.role = 'student'
    ORDER BY rc.uses_count DESC, rc.created_at DESC
    LIMIT 50
    `,
    [libraryId],
  );
