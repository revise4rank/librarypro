...
export async function listOwnerSeats(
  libraryId: string,
  filters?: { floorId?: string; status?: string },
) {
  return repository().listSeats(libraryId, filters);
}
...
export async function listOwnerPlanTemplates(libraryId: string) {
  return repository().listPlanTemplates(libraryId);
}

export async function createOwnerPlanTemplate(input: {
  libraryId: string;
  name: string;
  price: number;
  ...
  try {
    await client.query("BEGIN");
    const plan = await repo.createPlanTemplate(client, input);
    await client.query("COMMIT");
    return plan;
  } catch (error) {
    await client.query("ROLLBACK");
...
export async function updateOwnerPlanTemplate(input: {
  libraryId: string;
  planId: string;
  name?: string;
  price?: number;
  ...
  try {
    await client.query("BEGIN");
    const plan = await repo.updatePlanTemplate(client, {
      libraryId: input.libraryId,
      planId: input.planId,
    ...
    if (!plan) throw new AppError(404, "Plan template not found", "PLAN_NOT_FOUND");
    await client.query("COMMIT");
    return plan;
  } catch (error) {
    await client.query("ROLLBACK");
...
export async function listOwnerRooms(libraryId: string, floorId?: string) {
  return repository().listRooms(libraryId, floorId);
}
...
export async function createOwnerRoom(input: {
  ...
  try {
    await client.query("BEGIN");
    const room = await repo.createRoom(client, input);
    await client.query("COMMIT");
    return room;
  } catch (error) {
    await client.query("ROLLBACK");
...
export async function updateOwnerRoom(input: {
  ...
  try {
    await client.query("BEGIN");
    const room = await repo.updateRoom(client, {
      libraryId: input.libraryId,
      roomId: input.roomId,
      name: input.name ?? null,
...
export async function deleteOwnerRoom(libraryId: string, roomId: string) {
  const db = requireDb();
  const repo = repository();
  const client = await db.connect();
  try {
    await client.query("BEGIN");
    await repo.deleteRoom(client, libraryId, roomId);
    await client.query("COMMIT");
  } catch (error) {
    await client.query("ROLLBACK");
    throw error;
