...
export async function manualCheckinOwner(input: ManualCheckinInput) {
  const pool = requireDb();

  const assignment = await getActiveAssignment(input.libraryId, input.studentUserId);
  if (!assignment) {
    throw new AppError(404, "No active assignment found for this student", "ASSIGNMENT_NOT_FOUND");
  }

  const openVisit = await getOpenCheckin(input.libraryId, input.studentUserId);
  if (openVisit) {
    throw new AppError(409, "Student is already checked in", "ALREADY_CHECKED_IN");
  }

  const result = await pool.query<{ id: string; checked_in_at: string }>(
    `
      INSERT INTO checkins (
        library_id,
        student_user_id,
      ...
      VALUES ($1, $2, $3, $4, 'MANUAL'::checkin_mode, NULL, NOW(), NOW(), NULL)
      RETURNING id, checked_in_at::text
    `,
    [input.libraryId, input.studentUserId, assignment.id, assignment.seatId],
  );
