...
import { manualCheckinOwner } from "../services/checkin.service";
...
export async function listOwnerSeatsController(req: Request, res: Response) {
  ...
  const rows = await listOwnerSeats(libraryId, { floorId, status });
...
// ── Plan template controllers ─────────────────────────────────────────────────

export async function listOwnerPlanTemplatesController(req: Request, res: Response) {
  const { libraryId } = requireOwnerContext(req);
  const data = await listOwnerPlanTemplates(libraryId);
  res.json({ success: true, data });
}

export async function createOwnerPlanTemplateController(req: Request, res: Response) {
  const { libraryId, actorUserId } = requireOwnerContext(req);
  const { name, price, type, durationMonths, durationDays, shiftName, shiftStartHour, shiftEndHour } = req.body as {
    name: string; price: number; type: "MONTHLY" | "DAILY" | "SHIFT";
    durationMonths?: number; durationDays?: number; shiftName?: string;
    shiftStartHour?: number; shiftEndHour?: number;
  };
  if (!name || !price || !type) throw new AppError(400, "name, price, and type are required", "VALIDATION_ERROR");
  const data = await createOwnerPlanTemplate({
    libraryId, name: String(name).trim(), price: Number(price), type,
    durationMonths: durationMonths ?? null, durationDays: durationDays ?? null,
    shiftName: shiftName ?? null, shiftStartHour: shiftStartHour ?? null, shiftEndHour: shiftEndHour ?? null,
  });
  await createAuditLog({
    actorUserId, libraryId, action: "owner.plan_template.create",
    entityType: "plan_template", entityId: data.id,
    metadata: { name, type, price }, ipAddress: req.ip, userAgent: req.header("user-agent") ?? null,
  });
  res.status(201).json({ success: true, data });
}

export async function updateOwnerPlanTemplateController(req: Request, res: Response) {
  const { libraryId } = requireOwnerContext(req);
  const planId = paramValue(req.params.planId);
  const { name, price, isActive } = req.body as { name?: string; price?: number; isActive?: boolean };
  const data = await updateOwnerPlanTemplate({
    libraryId, planId,
...
export async function listOwnerRoomsController(req: Request, res: Response) {
  const { libraryId } = requireOwnerContext(req);
  const floorId = typeof req.query.floorId === "string" ? req.query.floorId : undefined;
  const data = await listOwnerRooms(libraryId, floorId);
  res.json({ success: true, data });
}

export async function createOwnerRoomController(req: Request, res: Response) {
  const { libraryId, actorUserId } = requireOwnerContext(req);
  const { floorId, name, capacity } = req.body as { floorId: string; name: string; capacity?: number };
  if (!floorId || !name) throw new AppError(400, "floorId and name are required", "VALIDATION_ERROR");
  const data = await createOwnerRoom({
    libraryId,
    floorId,
  ...
  await createAuditLog({
    ...
    entityId: data.id,
    metadata: { name, floorId },
    ipAddress: req.ip,
    userAgent: req.header("user-agent") ?? null,
  });
  emitLibraryEvent(libraryId, "seat.updated", { action: "room_created", roomId: data.id });
  res.status(201).json({ success: true, data });
}
...
export async function updateOwnerRoomController(req: Request, res: Response) {
  ...
  const data = await updateOwnerRoom({
    libraryId,
    roomId,
    name: name ? String(name).trim() : undefined,
...
export async function deleteOwnerRoomController(req: Request, res: Response) {
  const { libraryId } = requireOwnerContext(req);
  const roomId = paramValue(req.params.roomId);
  await deleteOwnerRoom(libraryId, roomId);
  emitLibraryEvent(libraryId, "seat.updated", { action: "room_deleted", roomId });
  res.json({ success: true });
}

export async function manualCheckinOwnerController(req: Request, res: Response) {
  const { libraryId, actorUserId } = requireOwnerContext(req);
  const { studentUserId } = req.body as { studentUserId: string };
  if (!studentUserId) {
    throw new AppError(400, "studentUserId is required", "STUDENT_REQUIRED");
  }
  const result = await manualCheckinOwner({ libraryId, studentUserId, actorUserId });
  emitLibraryEvent(libraryId, "checkin.created", { studentUserId, mode: "MANUAL" });
  res.status(201).json({ success: true, data: result });
}
