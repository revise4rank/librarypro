...
import { saveUploadedAdmissionDocument, saveUploadedAsset } from "../lib/storage";
...
export async function uploadAdmissionDocumentController(req: Request, res: Response) {
  if (!req.auth || req.auth.role !== "LIBRARY_OWNER") {
    throw new AppError(401, "Library owner authentication required", "OWNER_AUTH_REQUIRED");
  }
  ...
  const stored = await saveUploadedAdmissionDocument({
    file: req.file,
    libraryId,
  });
