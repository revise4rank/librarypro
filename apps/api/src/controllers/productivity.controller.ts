...
import {
  ...
  updatePlannerEntry,
  deletePlannerEntry,
  getOrCreateStudentReferralCode,
  getOwnerReferralStats,
  toggleFeedLike,
...
export async function createStudentBookController(req: Request, res: Response) {
  const { studentUserId, libraryId } = requireStudentContext(req);
  const { title, author, status, totalPages, currentPage, notes, startedAt } = req.body as {
    title: string;
    author?: string;
    status?: string;
    totalPages?: number;
  ...
  const data = await createStudentBook({
    studentUserId,
...
export async function updateStudentBookController(req: Request, res: Response) {
  const { studentUserId } = requireStudentContext(req);
  const bookId = paramValue(req.params.bookId, "BOOK_ID_REQUIRED");
  const { status, currentPage, totalPages, notes, finishedAt } = req.body as {
    status?: string;
    currentPage?: number;
    totalPages?: number;
    notes?: string;
  ...
  const data = await updateStudentBook({ id: bookId, studentUserId, status, currentPage, totalPages, notes, finishedAt });
  res.json({ success: true, data });
}

export async function deleteStudentBookController(req: Request, res: Response) {
  const { studentUserId } = requireStudentContext(req);
  const bookId = paramValue(req.params.bookId, "BOOK_ID_REQUIRED");
  await deleteStudentBook(bookId, studentUserId);
  res.json({ success: true });
}

// ── Study Planner Controllers ─────────────────────────────────────────────

export async function listPlannerWeekController(req: Request, res: Response) {
  const { studentUserId } = requireStudentContext(req);
  // weekStart: YYYY-MM-DD of Monday. Default to current week's Monday.
  let weekStart = typeof req.query.weekStart === "string" ? req.query.weekStart : null;
  if (!weekStart) {
    const now = new Date();
  ...
  const data = await listPlannerWeek(studentUserId, weekStart);
  res.json({ success: true, data, weekStart });
}

export async function listPlannerMonthController(req: Request, res: Response) {
  const { studentUserId } = requireStudentContext(req);
  const month = typeof req.query.month === "string"
    ? req.query.month
    : new Date().toISOString().slice(0, 7);
  const data = await listPlannerMonth(studentUserId, month);
  res.json({ success: true, data, month });
}

export async function createPlannerEntryController(req: Request, res: Response) {
  const { studentUserId } = requireStudentContext(req);
  ...
  const data = await createPlannerEntry({
    studentUserId,
    planDate,
    subject,
...
export async function updatePlannerEntryController(req: Request, res: Response) {
  const { studentUserId } = requireStudentContext(req);
  ...
  const data = await updatePlannerEntry({ id: entryId, studentUserId, actualMinutes, completed, notes, subject, targetMinutes });
  res.json({ success: true, data });
}

export async function deletePlannerEntryController(req: Request, res: Response) {
  const { studentUserId } = requireStudentContext(req);
  const entryId = paramValue(req.params.entryId, "ENTRY_ID_REQUIRED");
  await deletePlannerEntry(entryId, studentUserId);
  res.json({ success: true });
}

// ── Referral Controllers ──────────────────────────────────────────────────

export async function getStudentReferralCodeController(req: Request, res: Response) {
  const { studentUserId, libraryId } = requireStudentContext(req);
  const data = await getOrCreateStudentReferralCode(studentUserId, libraryId ?? null);
  res.json({ success: true, data });
}

export async function getOwnerReferralStatsController(req: Request, res: Response) {
  const { libraryId } = requireOwnerContext(req);
  const data = await getOwnerReferralStats(libraryId);
  res.json({ success: true, data });
}
