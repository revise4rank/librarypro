...
export class ProductivityRepository {
  ...
  async toggleFeedLike(postId: string, studentUserId: string): Promise<{ liked: boolean; likesCount: number }> {
    ...
    const existing = await this.pool.query<{ id: string }>(
      `SELECT id FROM feed_likes WHERE feed_post_id = $1 AND student_user_id = $2`,
    ...
    const countRow = await this.pool.query<{ count: string }>(
      `SELECT COUNT(*)::text AS count FROM feed_likes WHERE feed_post_id = $1`,
      [postId],
    );
    const likesCount = Number(countRow.rows[0]?.count ?? 0);

    // Update denormalized count column (added in migration 0024)
    await this.pool.query(
      `UPDATE student_feed_posts SET likes_count = $1 WHERE id = $2`,
      [likesCount, postId],
    ).catch(() => null); // Column may not exist yet in older DBs; safe to skip
  ...
  async listStudentBooks(studentUserId: string) {
    const result = await this.pool.query<{
      id: string;
      title: string;
      author: string | null;
      ...
      `
      SELECT id, title, author, status::text, total_pages, current_page,
             notes, started_at::text, finished_at::text, created_at::text
      FROM student_personal_books
      WHERE student_user_id = $1
      ORDER BY
        CASE status
          WHEN 'READING'   THEN 1
  ...
  async createStudentBook(input: {
    ...
    const result = await this.pool.query<{ id: string; created_at: string }>(
      `
      INSERT INTO student_personal_books
        (student_user_id, library_id, title, author, status, total_pages, current_page, notes, started_at)
      VALUES ($1, $2, $3, $4, $5::book_status, $6, $7, $8, $9::date)
  ...
  async updateStudentBook(input: {
    ...
    const params: unknown[] = [input.id, input.studentUserId];

    if (input.status !== undefined) {
      params.push(input.status);
      sets.push(`status = $${params.length}::book_status`);
    }
    if (input.currentPage !== undefined) {
      params.push(input.currentPage);
      sets.push(`current_page = $${params.length}`);
    }
    if (input.totalPages !== undefined) {
      params.push(input.totalPages);
      sets.push(`total_pages = $${params.length}`);
    }
    ...
    if (input.finishedAt !== undefined) {
      ...
      sets.push(`finished_at = $${params.length}::date`);
    }

    const result = await this.pool.query<{ id: string }>(
      `UPDATE student_personal_books SET ${sets.join(", ")} WHERE id = $1 AND student_user_id = $2 RETURNING id`,
      params,
    );
    return result.rows[0] ?? null;
  }

  async deleteStudentBook(id: string, studentUserId: string) {
    await this.pool.query(
      `DELETE FROM student_personal_books WHERE id = $1 AND student_user_id = $2`,
      [id, studentUserId],
    );
  }
  ...
  async listPlannerWeek(studentUserId: string, weekStart: string) {
    const result = await this.pool.query<{
      ...
      `
      SELECT id, plan_date::text, subject, target_minutes, actual_minutes, notes, completed
  ...
  async listPlannerMonth(studentUserId: string, month: string) {
    const result = await this.pool.query<{
      plan_date: string;
      total_target: number;
      total_actual: number;
      ...
      SELECT
        plan_date::text,
        SUM(target_minutes)::int AS total_target,
        SUM(actual_minutes)::int AS total_actual,
  ...
  async createPlannerEntry(input: {
    ...
    const result = await this.pool.query<{ id: string; plan_date: string }>(
      `
  ...
  async updatePlannerEntry(input: {
    ...
    if (input.actualMinutes !== undefined) {
      params.push(input.actualMinutes);
      sets.push(`actual_minutes = $${params.length}`);
    }
    ...
    if (input.targetMinutes !== undefined) {
      params.push(input.targetMinutes);
      sets.push(`target_minutes = $${params.length}`);
    }

    const result = await this.pool.query<{ id: string }>(
      `UPDATE study_plan_entries SET ${sets.join(", ")} WHERE id = $1 AND student_user_id = $2 RETURNING id`,
      params,
    );
    return result.rows[0] ?? null;
  }

  async deletePlannerEntry(id: string, studentUserId: string) {
    await this.pool.query(
      `DELETE FROM study_plan_entries WHERE id = $1 AND student_user_id = $2`,
      [id, studentUserId],
    );
  }
}
