...
export class OwnerOperationsRepository {
  ...
  async listSeats(libraryId: string, filters?: { floorId?: string; status?: string }) {
    const params: unknown[] = [libraryId];
    const conditions: string[] = ["s.library_id = $1"];

    if (filters?.floorId) {
      params.push(filters.floorId);
      conditions.push(`s.floor_id = $${params.length}`);
    }
    if (filters?.status) {
      params.push(filters.status);
      conditions.push(`s.status = $${params.length}::seat_status`);
    }
  ...
  async listPlanTemplates(libraryId: string) {
    const result = await this.pool.query<{
      id: string;
      name: string;
      price: string;
      ...
      `
      SELECT id::text, name, price::text, type::text, duration_months, duration_days,
             shift_name, shift_start_hour, shift_end_hour, is_active
      FROM plan_templates
  ...
  async createPlanTemplate(client: PoolClient, input: {
    libraryId: string;
    name: string;
    price: number;
    ...
    const result = await client.query<{ id: string; name: string }>(
      `
      INSERT INTO plan_templates (library_id, name, price, type, duration_months, duration_days, shift_name, shift_start_hour, shift_end_hour)
      VALUES ($1, $2, $3, $4::plan_type, $5, $6, $7, $8, $9)
      RETURNING id::text, name
      `,
      [
        input.libraryId, input.name, input.price, input.type,
        input.durationMonths ?? null, input.durationDays ?? null,
        input.shiftName ?? null, input.shiftStartHour ?? null, input.shiftEndHour ?? null,
      ],
    );
    return result.rows[0];
  }

  async updatePlanTemplate(client: PoolClient, input: {
    libraryId: string;
    planId: string;
    name?: string | null;
    price?: number | null;
    ...
    const result = await client.query<{ id: string; name: string }>(
      ...
      RETURNING id::text, name
      `,
      [input.libraryId, input.planId, input.name ?? null, input.price ?? null, input.isActive ?? null],
    );
    return result.rows[0] ?? null;
  }

  // ── End plan template operations ─────────────────────────────────────────

  // ── Room operations ────────────────────────────────────────────────────────

  async listRooms(libraryId: string, floorId?: string) {
    const result = await this.pool.query<{
      id: string;
      floor_id: string;
      name: string;
  ...
  async createRoom(client: PoolClient, input: {
    ...
    const result = await client.query<{ id: string; name: string; capacity: number }>(
      `
      INSERT INTO library_rooms (library_id, floor_id, name, capacity)
      VALUES ($1, $2, $3, $4)
      RETURNING id::text, name, capacity
      `,
      [input.libraryId, input.floorId, input.name, input.capacity],
    );
    return result.rows[0];
  }
  ...
  async updateRoom(client: PoolClient, input: {
    ...
    const result = await client.query<{ id: string; name: string; capacity: number }>(
      ...
      RETURNING id::text, name, capacity
      `,
      [input.libraryId, input.roomId, input.name ?? null, input.capacity ?? null],
    );
    return result.rows[0] ?? null;
  }

  async deleteRoom(client: PoolClient, libraryId: string, roomId: string) {
    // Detach seats from this room before deleting
    await client.query(
      `UPDATE seats SET room_id = NULL, updated_at = NOW() WHERE room_id = $1`,
      [roomId],
    );
    await client.query(
      `DELETE FROM library_rooms WHERE library_id = $1 AND id = $2`,
      [libraryId, roomId],
    );
  }
